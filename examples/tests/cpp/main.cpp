...
// const uint8_t some_index = 0x4;
    ...
    // [0xb][0x4][0x92][0xa5][0x49][0x33][0x3a][0x30][0x30][0x1][0xe5], 0x00
    // 92 A5 49 33 3A 30 30 01
    {
        std::vector<uint8_t> vs = {0x92, 0xa5, 0x49, 0x33, 0x3a, 0x30, 0x30, 0x01};
        bool b_success = false;
        ...
        const auto& p = Packetizer::encode(0x04, vs.data(), vs.size(), true);

        Packetizer::subscribe(0x04, [&](const uint8_t* data, const size_t size)
        {
            std::vector<uint8_t> rs;
            for (size_t i = 0; i < size; ++i) {
                std::cout << std::hex << (int)data[i] << " ";
                rs.emplace_back(data[i]);
            }
            std::cout << std::endl;
            b_success = (vs == rs);
            assert(b_success);
        ...
        Packetizer::feed(p.data.data(), p.data.size());
        assert(b_success);
        Packetizer::unsubscribe(0x04);
    }
    // [0x4][0x92][0xa4][0x49][0x33][0x3a][0x30][0x1][0x2b]
    // 92 A4 49 33 3A 30 01
    {
        std::vector<uint8_t> vs = {0x92, 0xa4, 0x49, 0x33, 0x3a, 0x30, 0x01};
        bool b_success = false;
